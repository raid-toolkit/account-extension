...
using Raid.Toolkit.AccountExtension;
using StaticDataWrapper = Raid.Toolkit.AccountExtension.StaticDataWrapper;
		...
		return new AccountDump()
		{
			...
			Heroes = heroes.Heroes.Values.Where(hero => !hero.Deleted).Select(hero =>
			{
				...
				Hero newHero = new()
				{
					...
					IsGuardian = academy != null && IsFactionGuardian(hero, academy),
					...
					Accuracy = heroType.Forms![0].UnscaledStats!.Accuracy,
					Attack = (int)Math.Round(heroType.Forms[0].UnscaledStats!.Attack * multiplier.multiplier),
					Defense = (int)Math.Round(heroType.Forms[0].UnscaledStats!.Defense * multiplier.multiplier),
					Health = (int)Math.Round(heroType.Forms[0].UnscaledStats!.Health * multiplier.multiplier) * 15,
					Speed = heroType.Forms[0].UnscaledStats!.Speed,
					Resistance = heroType.Forms[0].UnscaledStats!.Resistance,
					CriticalChance = heroType.Forms[0].UnscaledStats!.CriticalChance,
					CriticalDamage = heroType.Forms[0].UnscaledStats!.CriticalDamage,
					CriticalHeal = heroType.Forms[0].UnscaledStats!.CriticalHeal,
				};
